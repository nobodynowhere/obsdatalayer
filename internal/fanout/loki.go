...
func RegisterLoki(mux *http.ServeMux, cfg *config.Config, p *proxy.Proxy, client *http.Client, m *metrics.Metrics) {
	mux.HandleFunc("POST /api/{instance}/loki/push", func(w http.ResponseWriter, r *http.Request) {
		inst := getInstance(cfg, r, w, "loki")
		if inst == nil {
			return
		}
	...
	mux.HandleFunc("GET /api/{instance}/loki/query_range", func(w http.ResponseWriter, r *http.Request) {
		if inst := getInstance(cfg, r, w, "loki"); inst != nil {
