...
func RegisterMimir(mux *http.ServeMux, cfg *config.Config, p *proxy.Proxy, client *http.Client, m *metrics.Metrics) {
	mux.HandleFunc("POST /api/{instance}/mimir/push", func(w http.ResponseWriter, r *http.Request) {
		inst := getInstance(cfg, r, w, "mimir")
		if inst == nil {
			return
		}
	...
	mux.HandleFunc("GET /api/{instance}/mimir/query_range", func(w http.ResponseWriter, r *http.Request) {
		if inst := getInstance(cfg, r, w, "mimir"); inst != nil {
